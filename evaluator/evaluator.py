...
class Evaluator(object):
    ...
    def ref_similarity_0(self, path_texts_neg2pos, model_name):
        ...
        command = "python " + self.path_to_similarity_script +\
                  ...
                  " --reference_strs reference --reference_paths ~/learning/nlp/repo/mattes/evaluator/cleaned_test_1.txt " +\
                  "--output_path ~/learning/nlp/repo/mattes/save/" + model_name + "/0to1_sim.txt " +\
                  "--store_scores"
        print(command)
        os.system(command)
        if os.path.exists(path_texts_neg2pos + ".pp_scores"):
            sim_file = np.loadtxt(path_texts_neg2pos + ".pp_scores")
            return sim_file.mean()
        else:
            return np.zeros(1).mean()

    def ref_similarity_1(self, path_texts_pos2neg, model_name):
        command = "python " + self.path_to_similarity_script +\
                  " --generated_path " + path_texts_pos2neg  +\
                  " --reference_strs reference --reference_paths ~/learning/nlp/repo/mattes/evaluator/cleaned_test_0.txt " +\
                  "--output_path ~/learning/nlp/repo/mattes/save/" + model_name + "/1to0_sim.txt " +\
                  "--store_scores"
        print(command)
        os.system(command)
        if os.path.exists(path_texts_pos2neg + ".pp_scores"):
    ...
    def ref_bartscore_0(self, texts_neg2pos):
        ...
        with open('/home/arthur/learning/nlp/repo/mattes/metricas/ablat_shake_no_para_and_kd_2000_bscore.txt', 'a+') as fl:
